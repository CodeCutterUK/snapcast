    ...
    try
    {
        ...
        if (request->method().find("Client.") == 0)
        {
            ...
            else if (request->method() == "Client.IncVolume")
			{
                ...
                if (clientInfo->config.volume.percent > 100) clientInfo->config.volume.percent = 100;
				result["volume"] = clientInfo->config.volume.toJson();
				notification.reset(new jsonrpcpp::Notification("Client.OnVolumeChanged",
                                                               jsonrpcpp::Parameter("id", clientInfo->id, "volume", clientInfo->config.volume.toJson())));
			}
			else if (request->method() == "Client.DecVolume")
			{
				...
				if (request->params().has("incrementAmount"))
					incrementAmount = request->params().get("incrementAmount");				
                if ((int32_t)clientInfo->config.volume.percent - incrementAmount < 0)
                    clientInfo->config.volume.percent = 0;
                else
				    clientInfo->config.volume.percent -= incrementAmount;
                if (clientInfo->config.volume.percent > 100) clientInfo->config.volume.percent = 100;
				result["volume"] = clientInfo->config.volume.toJson();
